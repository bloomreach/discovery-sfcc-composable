...
import {SORT_OPTIONS} from '@bloomreach/constants'
import useNavigation from '~/app/hooks/use-navigation'
import LoadingSpinner from '@salesforce/retail-react-app/app/components/loading-spinner'
import {useWishList} from '@salesforce/retail-react-app/app/hooks/use-wish-list'
import {isHydrated, isServer} from '@salesforce/retail-react-app/app/utils/utils'
...
import {
    getEfqString,
    getSortReplacedPath,
    getSearchParamsSortValue,
    navigateRedirect
} from '@bloomreach/utils.js'
import {useGetProductsByFilters, useGetContent} from '@bloomreach/data-access/client/queries'
import {useBloomreachAnalytics} from '@bloomreach/hooks/analytics'
import SelectTabButton from '@bloomreach/components/SelectTabButtons'
import ContentList from '@bloomreach/components/ContentList/index'
import {CATALOG_NAME} from '@bloomreach/constants'

// NOTE: You can ignore certain refinements on a template level by updating the below
// list of ignored refinements.
// Bloomreach Note: This is left as-is from PWAKit Template
...
const ProductList = (props) => {
    ...
    if (!isEmpty(searchParams?.refine)) {
        const {price, ...attributesExceptPrice} = searchParams.refine
        ...
        queryParams.efq = getEfqString(attributesExceptPrice)
    }

    if (searchParams?.sort && searchParams?.sort !== SORT_OPTIONS[0].id) {
        queryParams.sort = getSearchParamsSortValue(searchParams)
    }
    ...
    const [isShowingContentSearch, setIsShowingContentSearch] = useState(
        searchParams?.content || false
    )
    const contentResultsCount = contentData?.response?.numFound

    const contentPageUrls = usePageUrls({
        total: contentResultsCount,
        limit: DEFAULT_LIMIT_VALUES[0]
    })

    const toggleTab = (isShowingContent) => {
        ...
        if (isShowingContent) {
            searchParamsCopy.content = true
        }

        if (searchParamsCopy.content && !isShowingContent) {
            delete searchParamsCopy.content
        }

        const oldStringifiedParams = stringifySearchParams(searchParams)
        const newStringifiedParams = stringifySearchParams(searchParamsCopy)

        // This is needed to avoid pushing the same exact state in history
        if (oldStringifiedParams !== newStringifiedParams) {
            navigate(`/search?${newStringifiedParams}`)
        }
    }

    useEffect(() => {
        setIsShowingContentSearch(searchParams?.content)
    ...
    // Reset scroll position when `isLoadingFinished` becomes `true`.
    useEffect(() => {
        !isLoadingFinished && window.scrollTo(0, 0)
    }, [isLoadingFinished])
    ...
    const showNoResults = !isLoading && productSearchResult && !productSearchResult?.hits?.length
    ...
    const pageUrls = usePageUrls({total, limit: DEFAULT_LIMIT_VALUES[0]})
    ...
    useEffect(() => {
        ...
        const cat = category.parentCategoryTree?.reduce((acc, el, index, array) => {
            const delim = index === array.length - 1 ? '' : '|'
            return acc + el?.name + delim
        }, '')
                ...
                <>
                    ...
                    {isSearch && contentResultsCount > 0 && (
                        <Flex w="100%" mb="20px">
                            <Box width="50%">
                                <SelectTabButton
                                    isActive={!searchParams?.content}
                                    label="Products"
                                    count={productSearchResult?.total}
                        ...
                        <>
                            ...
                            <Grid templateColumns={{base: '1fr', md: '280px 1fr'}} columnGap={6}>
                                ...
                                <Box>
                                    ...
                                    >
                                        {isHydrated() &&
                                        (!isLoadingFinished || !productSearchResult)
...
const Sort = ({sortUrls, productSearchResult, basePath, ...otherProps}) => {
    ...
    return (
        <FormControl data-testid="sf-product-list-sort" id="page_sort" width="auto" {...otherProps}>
            <Select
                value={getSortReplacedPath(basePath)}
                onChange={({target}) => {
                    history.push(target.value)
                }}
                height={11}
